...
def score_kc(
    ...
    if variant == "count":
        # Максимизируем число KC перешедших порог → берём ближайшие к порогу снизу
        # KC уже выше порога исключаем (нечего учить), остальные: меньше gap = выше приоритет
        if gap == 0.0:
            return 0.0
        base = 1.0 / (gap + 0.05)
    elif variant == "mass":
        ...
        # Бонус за KC которые реально не тронуты системой (cold_start ≤ 0.10)
        frontier = 2.0 if m < 0.10 else 1.0
        base = (1.0 - m) * frontier
    else:
        base = 1.0 / (gap + 0.05)
