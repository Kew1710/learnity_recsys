"""
Seed-данные: методический граф по математике 5–9 класс (ОГЭ).

Таксономия взята из реального перечня тем по классам:
  5 класс — арифметика + геометрия (смешано)
  6 класс — дроби, рациональные числа, основы алгебры
  7 класс — Алгебра (Колягин) + Геометрия (Атанасян)
  8 класс — Алгебра + Геометрия
  9 класс — Алгебра + Геометрия

kc_type:
  declarative — знать понятие/определение
  procedural  — уметь выполнить вычисление/преобразование
  applied     — применить в контексте задачи

subject:
  arithmetic  — арифметика и числа (5–6 кл.)
  algebra     — алгебраические структуры, уравнения, функции (6–9 кл.)
  geometry    — геометрия (5–9 кл.)
  statistics  — вероятность и статистика
...
# Период забывания по предмету (дней до падения освоенности вдвое).
# Процедурные навыки арифметики держатся дольше абстрактных концептов.
...
EDGES = [
    ...
    ("kc_divisibility",           "kc_fractions_operations",    0.4),  # weak: GCD/LCM helpful but not blocking
